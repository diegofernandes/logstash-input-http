...
class JwtToken
  ...
  def call(env)
    if path_matches_excluded_path?(env)
      @app.call(env)
    elsif (missing_auth_header?(env) && missing_auth_path?(env))
      @logger.error("Missing Authorization token. request => #{env}") if @logger
      return_error('Missing Authorization token')
    elsif (invalid_auth_header?(env) && invalid_auth_path_token?(env))
      @logger.error("Invalid Authorization token. request => #{env}") if @logger
      return_error('Invalid Authorization token')
    else
      verify_token(env)
    end
  end
  ...
  def verify_token(env)
    ...
    token = BEARER_TOKEN_REGEX.match(env['HTTP_AUTHORIZATION'])[1] if !missing_auth_header?(env)
    token = PATH_TOKEN_REGEX.match(env['REQUEST_URI'])[1] if !missing_auth_path?(env)

    begin
      decoded_token = JWT.decode token, @secret
      env['metadata'] = decoded_token.first
      env['metadata']['headers'] = decoded_token.last
      @app.call(env)
    rescue => e
  ...
  def valid_auth_path_token?(env)
    env['REQUEST_URI']=~ PATH_TOKEN_REGEX
  end
  ...
  def invalid_auth_path_token?(env)
    !valid_auth_path_token?(env)
  end
  ...
  def missing_auth_path?(env)
    env['REQUEST_URI'].nil? || env['REQUEST_URI'].empty? || invalid_auth_path_token?(env)
  end
